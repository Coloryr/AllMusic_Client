...
public class MiniMessage {
    ...
    // 装饰器映射
    private static final Map<String, TextFormatting> DECORATION_MAP = new HashMap<>();
    static {
        DECORATION_MAP.put("bold", TextFormatting.BOLD);
        DECORATION_MAP.put("italic", TextFormatting.ITALIC);
        DECORATION_MAP.put("underlined", TextFormatting.UNDERLINE);
        DECORATION_MAP.put("strikethrough", TextFormatting.STRIKETHROUGH);
        DECORATION_MAP.put("obfuscated", TextFormatting.OBFUSCATED);
    }
    ...
    public static TextComponent parse(String input) {
        TextComponent root = new StringTextComponent("");
    ...
    private static void handleColorTag(StyleContext style, String params, String value) {
        if (value != null) {
            ...
            Color textColor = parseColor(value);
            if (textColor != null) {
            ...
            if (params.equalsIgnoreCase("rgb")) {
                ...
                if (parts.length == 3) {
                    try {
                        ...
                        style.color = Color.fromRgb((r << 16) | (g << 8) | b);
                        // 清除渐变、过渡和彩虹效果
                        style.gradient = false;
                        style.transition = false;
                        style.rainbow = false;
    ...
    private static void handleShadowTag(StyleContext style, String params, String value) {
        if (value == null) {
            ...
            // 自定义阴影颜色
            Color shadowColor = parseColor(value);
            if (shadowColor != null) {
                style.shadow = true;
    ...
    private static void handleDecorationTag(StyleContext style, String params, String value) {
        ...
        TextFormatting formatting = DECORATION_MAP.get(decorationName);
        if (formatting != null) {
    ...
    private static void handleGradientTag(StyleContext style, String params, String value) {
        ...
        if (colors.length == 2) {
            Color start = parseColor(colors[0]);
            Color end = parseColor(colors[1]);
            ...
            if (color1.length == 3 && color2.length == 3) {
                try {
                    ...
                    style.transition = false;
                    style.rainbow = false;
                    style.gradientStart = Color.fromRgb((r1 << 16) | (g1 << 8) | b1);
                    style.gradientEnd = Color.fromRgb((r2 << 16) | (g2 << 8) | b2);
                } catch (NumberFormatException e) {}
            }
        }
    }
        ...
        if (colors.length == 2) {
            ...
            if (color1.length == 3 && color2.length == 3) {
                try {
                    ...
                    style.gradient = false;
                    style.rainbow = false;
                    style.transitionStart = Color.fromRgb((r1 << 16) | (g1 << 8) | b1);
                    style.transitionEnd = Color.fromRgb((r2 << 16) | (g2 << 8) | b2);
                } catch (NumberFormatException e) {}
            }
        }
    }
    ...
    private static Color parseColor(String colorStr) {
        ...
        // 检查是否为预定义颜色名称
        TextFormatting formatting = COLOR_MAP.get(colorStr.toLowerCase(Locale.ROOT));
        if (formatting != null) {
            return Color.fromLegacyFormat(formatting);
        }
        ...
        if (colorStr.startsWith("#")) {
            try {
                if (colorStr.length() == 7) {
                    ...
                    return Color.parseColor(expanded);
                }
            } catch (Exception e) {
                // 忽略解析错误
            }
     ...
     * 添加文本到组件
     */
    private static void appendText(TextComponent root, String text, StyleContext styleContext) {
        ...
        // 普通文本
            ...
            TextComponent component = new StringTextComponent(text);
     ...
     * 带渐变效果的文本追加（逐字符颜色变化）
     */
    private static void appendGradientText(TextComponent root, String text, StyleContext styleContext) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            Color gradientColor = interpolateColor(styleContext.gradientStart, styleContext.gradientEnd, ratio);
...
//            }
            ...
            root.append(new StringTextComponent(String.valueOf(c)).withStyle(charStyle));
        }
    }
     ...
     * 带过渡效果的文本追加（整体颜色过渡）
     */
    private static void appendTransitionText(TextComponent root, String text, StyleContext styleContext) {
        Style baseStyle = styleContext.buildBaseStyle();

        // 过渡效果：为整个文本块创建一个颜色过渡
        TextComponent transitionComponent = new StringTextComponent(text);
     ...
     * 带彩虹效果的文本追加
     */
    private static void appendRainbowText(TextComponent root, String text, StyleContext styleContext) {
        int length = text.length();
        Style baseStyle = styleContext.buildBaseStyle();
        ...
        for (int i = 0; i < length; i++) {
            ...
            Color rainbowColor = Color.fromRgb(rgb & 0xFFFFFF);
...
//            }
    ...
    private static Color interpolateColor(Color start, Color end, float ratio) {
        ...
        return Color.fromRgb((r << 16) | (g << 8) | b);
    }
    ...
    private static class StyleContext {
        ...
        Map<TextFormatting, Boolean> decorations = new HashMap<>();
        ...
        Color transitionStart = null;
        Color transitionEnd = null;
        String font = null;
        ...
        Style buildBaseStyle() {
            ...
            // 应用装饰器
            for (Map.Entry<TextFormatting, Boolean> entry : decorations.entrySet()) {
                if (entry.getValue()) {
