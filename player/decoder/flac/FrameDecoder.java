...
public final class FrameDecoder {
    ...
    public FrameInfo readFrame(int[][] outSamples, int outOffset) throws IOException {
        ...
        if (currentBlockSize != -1) throw new IllegalStateException("Concurrent call");
        ...
        if (outOffset < 0 || outOffset > outSamples[0].length) throw new IndexOutOfBoundsException();
        if (outSamples.length < meta.numChannels)
            throw new IllegalArgumentException("Output array too small for number of channels");
        if (outOffset > outSamples[0].length - currentBlockSize) throw new IndexOutOfBoundsException();

        // Do the hard work
        decodeSubframes(expectedSampleDepth, meta.channelAssignment, outSamples, outOffset);

        // Read padding and footer
        if (in.readUint((8 - in.getBitPosition()) % 8) != 0) throw new DataFormatException("Invalid padding bits");
        int computedCrc16 = in.getCrc16();
        if (in.readUint(16) != computedCrc16) throw new DataFormatException("CRC-16 mismatch");

        // Handle frame size and miscellaneous
        long frameSize = in.getPosition() - startByte;
        if (frameSize < 10) throw new AssertionError();
        if ((int) frameSize != frameSize) throw new DataFormatException("Frame size too large");
    ...
    private void decodeSubframes(int sampleDepth, int chanAsgn, int[][] outSamples, int outOffset) throws IOException {
        // Check arguments
        if (sampleDepth < 1 || sampleDepth > 32) throw new IllegalArgumentException();
        if ((chanAsgn >>> 4) != 0) throw new IllegalArgumentException();
        ...
        if (0 <= chanAsgn && chanAsgn <= 7) {
            ...
            if (chanAsgn == 8) { // Left-side stereo
                for (int i = 0; i < currentBlockSize; i++) temp1[i] = temp0[i] - temp1[i];
            } else if (chanAsgn == 9) { // Side-right stereo
                for (int i = 0; i < currentBlockSize; i++) temp0[i] += temp1[i];
    ...
    private void decodeSubframe(int sampleDepth, long[] result) throws IOException {
        ...
        if (sampleDepth < 1 || sampleDepth > 33) throw new IllegalArgumentException();
        if (result.length < currentBlockSize) throw new IllegalArgumentException();

        // Read header fields
        if (in.readUint(1) != 0) throw new DataFormatException("Invalid padding bit");
        int type = in.readUint(6);
        int shift = in.readUint(1); // Also known as "wasted bits-per-sample"
        if (shift == 1) {
            while (in.readUint(1) == 0) { // Unary coding
                if (shift >= sampleDepth) throw new DataFormatException("Waste-bits-per-sample exceeds sample depth");
                shift++;
            }
        }
        if (!(0 <= shift && shift <= sampleDepth)) throw new AssertionError();
        sampleDepth -= shift;
        ...
        if (type == 0) // Constant coding
            ...
            for (int i = 0; i < currentBlockSize; i++) result[i] = in.readSignedInt(sampleDepth);
        } else if (8 <= type && type <= 12) decodeFixedPredictionSubframe(type - 8, sampleDepth, result);
        else if (32 <= type && type <= 63) decodeLinearPredictiveCodingSubframe(type - 31, sampleDepth, result);
        else throw new DataFormatException("Reserved subframe type");

        // Add trailing zeros to each sample
        if (shift > 0) {
            for (int i = 0; i < currentBlockSize; i++) result[i] <<= shift;
        }
    }
    ...
    private void decodeFixedPredictionSubframe(int predOrder, int sampleDepth, long[] result) throws IOException {
        ...
        Objects.requireNonNull(result);
        if (sampleDepth < 1 || sampleDepth > 33) throw new IllegalArgumentException();
        if (predOrder < 0 || predOrder >= FIXED_PREDICTION_COEFFICIENTS.length) throw new IllegalArgumentException();
        if (predOrder > currentBlockSize) throw new DataFormatException("Fixed prediction order exceeds block size");
        if (result.length < currentBlockSize) throw new IllegalArgumentException();
    ...
    private void decodeLinearPredictiveCodingSubframe(int lpcOrder, int sampleDepth, long[] result) throws IOException {
        ...
        Objects.requireNonNull(result);
        if (sampleDepth < 1 || sampleDepth > 33) throw new IllegalArgumentException();
        if (lpcOrder < 1 || lpcOrder > 32) throw new IllegalArgumentException();
        if (lpcOrder > currentBlockSize) throw new DataFormatException("LPC order exceeds block size");
        if (result.length < currentBlockSize) throw new IllegalArgumentException();

        // Read non-Rice-coded warm-up samples
        for (int i = 0; i < lpcOrder; i++) result[i] = in.readSignedInt(sampleDepth);

        // Read parameters for the LPC coefficients
        int precision = in.readUint(4) + 1;
        if (precision == 16) throw new DataFormatException("Invalid LPC precision");
        int shift = in.readSignedInt(5);
        if (shift < 0) throw new DataFormatException("Invalid LPC shift");

        // Read the coefficients themselves
        int[] coefs = new int[lpcOrder];
        for (int i = 0; i < coefs.length; i++) coefs[i] = in.readSignedInt(precision);
    ...
    // This method reads all the arguments and the field currentBlockSize, only writes to result, and has no other side
    // effects.
    ...
    // Because coefs.length <= 32 and each coefficient fits in a signed int15 (both according to the FLAC
    // specification),
    ...
    private void restoreLpc(long[] result, int[] coefs, int sampleDepth, int shift) {
        ...
        if (result.length < currentBlockSize) throw new IllegalArgumentException();
        if (sampleDepth < 1 || sampleDepth > 33) throw new IllegalArgumentException();
        if (shift < 0 || shift > 63) throw new IllegalArgumentException();
        long lowerBound = (-1) << (sampleDepth - 1);
        long upperBound = -(lowerBound + 1);

        for (int i = coefs.length; i < currentBlockSize; i++) {
            long sum = 0;
            for (int j = 0; j < coefs.length; j++) sum += result[i - 1 - j] * coefs[j];
    ...
    private void readResiduals(int warmup, long[] result) throws IOException {
        // Check and handle arguments
        Objects.requireNonNull(result);
        if (warmup < 0 || warmup > currentBlockSize) throw new IllegalArgumentException();
        if (result.length < currentBlockSize) throw new IllegalArgumentException();

        int method = in.readUint(2);
        if (method >= 2) throw new DataFormatException("Reserved residual coding method");
        ...
        for (int inc = currentBlockSize >>> partitionOrder, partEnd = inc, resultIndex = warmup; partEnd
            ...
            if (param == escapeParam) {
                ...
                for (; resultIndex < partEnd; resultIndex++) result[resultIndex] = in.readSignedInt(numBits);
            } else {
