...
final class SynthesisFilter {
    ...
    private void compute_pcm_samples0() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[dvp] * dp[0]) + (vp[15 + dvp] * dp[1])
                    + (vp[14 + dvp] * dp[2])
                    + (vp[13 + dvp] * dp[3])
                    + (vp[12 + dvp] * dp[4])
                    + (vp[11 + dvp] * dp[5])
    ...
    private void compute_pcm_samples1() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[1 + dvp] * dp[0]) + (vp[dvp] * dp[1])
                    + (vp[15 + dvp] * dp[2])
                    + (vp[14 + dvp] * dp[3])
                    + (vp[13 + dvp] * dp[4])
                    + (vp[12 + dvp] * dp[5])
    ...
    private void compute_pcm_samples2() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[2 + dvp] * dp[0]) + (vp[1 + dvp] * dp[1])
                    + (vp[dvp] * dp[2])
                    + (vp[15 + dvp] * dp[3])
                    + (vp[14 + dvp] * dp[4])
                    + (vp[13 + dvp] * dp[5])
    ...
    private void compute_pcm_samples3() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[3 + dvp] * dp[0]) + (vp[2 + dvp] * dp[1])
                    + (vp[1 + dvp] * dp[2])
                    + (vp[dvp] * dp[3])
                    + (vp[15 + dvp] * dp[4])
                    + (vp[14 + dvp] * dp[5])
    ...
    private void compute_pcm_samples4() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[4 + dvp] * dp[0]) + (vp[3 + dvp] * dp[1])
                    + (vp[2 + dvp] * dp[2])
                    + (vp[1 + dvp] * dp[3])
                    + (vp[dvp] * dp[4])
                    + (vp[15 + dvp] * dp[5])
    ...
    private void compute_pcm_samples5() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[5 + dvp] * dp[0]) + (vp[4 + dvp] * dp[1])
                    + (vp[3 + dvp] * dp[2])
                    + (vp[2 + dvp] * dp[3])
                    + (vp[1 + dvp] * dp[4])
                    + (vp[dvp] * dp[5])
    ...
    private void compute_pcm_samples6() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[6 + dvp] * dp[0]) + (vp[5 + dvp] * dp[1])
                    + (vp[4 + dvp] * dp[2])
                    + (vp[3 + dvp] * dp[3])
                    + (vp[2 + dvp] * dp[4])
                    + (vp[1 + dvp] * dp[5])
    ...
    private void compute_pcm_samples7() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[7 + dvp] * dp[0]) + (vp[6 + dvp] * dp[1])
                    + (vp[5 + dvp] * dp[2])
                    + (vp[4 + dvp] * dp[3])
                    + (vp[3 + dvp] * dp[4])
                    + (vp[2 + dvp] * dp[5])
    ...
    private void compute_pcm_samples8() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[8 + dvp] * dp[0]) + (vp[7 + dvp] * dp[1])
                    + (vp[6 + dvp] * dp[2])
                    + (vp[5 + dvp] * dp[3])
                    + (vp[4 + dvp] * dp[4])
                    + (vp[3 + dvp] * dp[5])
    ...
    private void compute_pcm_samples9() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[9 + dvp] * dp[0]) + (vp[8 + dvp] * dp[1])
                    + (vp[7 + dvp] * dp[2])
                    + (vp[6 + dvp] * dp[3])
                    + (vp[5 + dvp] * dp[4])
                    + (vp[4 + dvp] * dp[5])
    ...
    private void compute_pcm_samples10() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[10 + dvp] * dp[0]) + (vp[9 + dvp] * dp[1])
                    + (vp[8 + dvp] * dp[2])
                    + (vp[7 + dvp] * dp[3])
                    + (vp[6 + dvp] * dp[4])
                    + (vp[5 + dvp] * dp[5])
    ...
    private void compute_pcm_samples11() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[11 + dvp] * dp[0]) + (vp[10 + dvp] * dp[1])
                    + (vp[9 + dvp] * dp[2])
                    + (vp[8 + dvp] * dp[3])
                    + (vp[7 + dvp] * dp[4])
                    + (vp[6 + dvp] * dp[5])
    ...
    private void compute_pcm_samples12() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[12 + dvp] * dp[0]) + (vp[11 + dvp] * dp[1])
                    + (vp[10 + dvp] * dp[2])
                    + (vp[9 + dvp] * dp[3])
                    + (vp[8 + dvp] * dp[4])
                    + (vp[7 + dvp] * dp[5])
    ...
    private void compute_pcm_samples13() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[13 + dvp] * dp[0]) + (vp[12 + dvp] * dp[1])
                    + (vp[11 + dvp] * dp[2])
                    + (vp[10 + dvp] * dp[3])
                    + (vp[9 + dvp] * dp[4])
                    + (vp[8 + dvp] * dp[5])
    ...
    private void compute_pcm_samples14() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            pcm_sample = ((vp[14 + dvp] * dp[0]) + (vp[13 + dvp] * dp[1])
                    + (vp[12 + dvp] * dp[2])
                    + (vp[11 + dvp] * dp[3])
                    + (vp[10 + dvp] * dp[4])
                    + (vp[9 + dvp] * dp[5])
    ...
    private void compute_pcm_samples15() {
        ...
        for (int i = 0; i < 32; i++) {
            float pcm_sample;
            final float[] dp = d16[i];
            pcm_sample = ((vp[15 + dvp] * dp[0]) + (vp[14 + dvp] * dp[1])
                    + (vp[13 + dvp] * dp[2])
                    + (vp[12 + dvp] * dp[3])
                    + (vp[11 + dvp] * dp[4])
                    + (vp[10 + dvp] * dp[5])
